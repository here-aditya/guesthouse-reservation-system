...
class Distributor extends CI_Model {
	...
	{
		$this->db->select("{$this->_table}.name,
							{$this->_table}.city_id,
							{$this->_table}.contact,
							{$this->_table}.address,
    ...
    {	
		$data = array('name' => trim($post_data['txt_distributor']), 
					  'city_id' => trim($post_data['sel_city']),
					  'contact' => trim($post_data['txt_contact']),
	...
	public function updateEntry($post_data)
	{
		...
		try
		{
				$this->db->where('id', $post_data['edit_distributor_id']);
